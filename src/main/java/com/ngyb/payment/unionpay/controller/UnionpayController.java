package com.ngyb.payment.unionpay.controller;

import com.google.gson.Gson;
import com.ngyb.payment.unionpay.base.DemoBase;
import com.ngyb.payment.unionpay.config.SDKConfig;
import com.ngyb.payment.unionpay.service.AcpService;
import com.ngyb.payment.unionpay.utils.LogUtil;
...
public class UnionpayController {
    String merId = "777290058184403";//商户号码，请改成自己申请的商户号或者open上注册得来的777商户号测试
    String txnAmt = "1";//交易金额 单位为分，不能带小数点
    String orderId = String.valueOf(System.currentTimeMillis());//商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
    String txnTime = formatTime(new Date());//订单发送时间，取系统时间，格式为yyyyMMddHHmmss，必须取当前时间，否则会报txnTime无效

    @RequestMapping(value = "getacptn")
    public String getTn() {
...
//		contentData.put("reqReserved", Base64.encodeBase64String("任意格式的信息都可以".toString().getBytes(DemoBase.encoding)));
        ...
        LogUtil.writeLog("返回的数据：" + new Gson().toJson(rspData));
